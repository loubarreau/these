...
Dans ce chapitre, nous rappellerons les développements et les principaux résultats du modèle de Fano pour l'autoionisation, phénomène qui se produit lorsqu'un système est excité dans un état d'énergie supérieure à son potentiel d'ionisation.
...
On considère un système atomique constitué d'un état du continuum $\ket{\psi_{E'}}$ et d'un état discret (pseudo-) lié $\ket{\varphi}$, couplés par $V_{E'}$. On a:
\begin{align}
...
\begin{align}
a_E \: E_\varphi + \int b_{E'} V^*_{E'} \rmd E' & = E \: a_E \\
a_E \: V_{E'} + b_{E'} \: E' & = E \: b_{E'}
\end{align} 
...
\end{equation}
l'état lié $\ket{\varphi}$ modifié un ajout d'états du continuum dû à l'interaction de configuration.

La section efficace $\sigma$ s'exprime alors
\begin{align}
\sigma & \propto |\bra{\Psi_E} \hat{T} \ket{g}|^2 \\
& \propto |\frac{\sin \Delta}{\pi V^*_E} \bra{\Phi} \hat{T} \ket{g} -  \cos \Delta \bra{\psi_E} \hat{T} \ket{g}|^2 \\
& \propto |\bra{\psi_E} \hat{T} \ket{g}|^2 \: \: |\frac{\sin \Delta}{\pi V^*_E} \frac{\bra{\Phi} \hat{T} \ket{g}}{\bra{\psi_E} \hat{T} \ket{g}} -  \cos \Delta |^2
...
\end{equation}
$q$ caractérise donc le rapport entre les probabilités de transition vers l'état discret (modifié par le continuum) et vers le continuum (non perturbé). 

\begin{figure}
\centering
\def\svgwidth{0.7\textwidth}
\import{Figures/DelaisPI/}{ProfilsFano.pdf_tex}
\caption{Allure de la section efficace d'absorption pour plusieurs valeurs du paramètre $q$.}
...
Une expression différente de la section efficace permet d'interpréter physiquement le phénomène de résonance de Fano:
\begin{equation}
\sigma = \sigma_0 \: \frac{\left(q + \epsilon \right) ^2}{1+\epsilon^2} = \sigma_0 \left( \underbrace{1}_\textrm{Continuum} + \underbrace{\frac{q^2 - 1}{1 + \epsilon^2}}_\textrm{\'Etat discret} + \underbrace{\frac{2 q \epsilon}{1 + \epsilon^2}}_\textrm{Couplage} \right)
\end{equation}
