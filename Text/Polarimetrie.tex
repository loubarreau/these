...
\end{equation}
Les paramètres de Stokes sont calculés à partir de l'accélération dans le domaine des fréquences, selon la méthode décrite par \mycite{AntoinePRA1997}:
\begin{align}
& S_0 = I(0,0) + I(\pi/2, 0) \\
& S_1 = I(0,0) - I(\pi/2,0) \\
...
\begin{equation}
I(\theta, \eta) = \int \int |a_x(r,t) \cos(\theta) + a_y(r,t) \sin(\theta) \rme^{i \eta} |^2 \rmd r \rmd t
\end{equation}
L'analyse est restreinte aux harmoniques au-dessus du potentiel d'ionisation du milieu considéré, les harmoniques sous le seuil possédant des propriétés particulières \mycite{MedisauskasPRL2015}. Dans un calcul à trois dimensions, la seule différence attendue est la diffusion du paquet d'onde électronique du continuum dans la dimension perpendiculaire au plan de polarisation du champ, identique pour les orbitales $p_{-1}$ et $p_{+1}$. Les résultats devraient donc être identiques dans un calcul à trois dimensions \mycite{MedisauskasPRL2015}.
